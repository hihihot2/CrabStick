...
[
	...
	{name: "${hotel.name }", address: "${hotel.address }", lat: "${hotel.latitude }", lng: "${hotel.longitude }", img: "http://media.expedia.com${hotel.thumbnailUrl}", type: 1}<c:if test="${!status.last }">,</c:if> 
	</c:forEach>
	...
	{name: "${venue.name }", address: "${venue.location.address}", lat: "${venue.location.lat}", lng: "${venue.location.lng }", img: "", type: 2}<c:if test="${!status.last }">,</c:if>
	</c:forEach>
	...
	{name: "", address: "", lat: "", lng: "", img: "", type: 6}<c:if test="${!status.last }">,</c:if>
	</c:forEach>
]
