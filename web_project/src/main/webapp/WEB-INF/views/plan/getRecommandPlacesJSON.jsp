...
[
	...
	<c:if test="${!empty HOTELS && (!empty VENUES || !empty SIGHTS || !empty SHOPPINGS || !empty RESTS || !empty NATURES || !empty RECOMMENDS)}">,</c:if>
	...
	<c:if test="${!empty VENUES && (!empty SIGHTS || !empty SHOPPINGS || !empty RESTS || !empty NATURES || !empty RECOMMENDS)}">,</c:if>
	...
	<c:if test="${!empty SIGHTS && (!empty SHOPPINGS || !empty RESTS || !empty NATURES || !empty RECOMMENDS)}">,</c:if>
	...
	<c:if test="${!empty SHOPPINGS && (!empty RESTS || !empty NATURES || !empty RECOMMENDS)}">,</c:if>
	...
	<c:if test="${!empty RESTS && (!empty NATURES || !empty RECOMMENDS)}">,</c:if>
	...
	<c:if test="${!empty NATURES && !empty RECOMMENDS}">,</c:if>
	<c:forEach var="recommend" items="${RECOMMENDS }" varStatus="status">
	{name: "${recommend.title}", address: "${recommend.addr1}", lat: "${recommend.mapy}", lng: "${recommend.mapx}", img: "${recommend.imgURL}", type: 6}<c:if test="${!status.last }">,</c:if>
	</c:forEach>
]
